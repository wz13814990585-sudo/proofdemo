...
## Stage 13.1 — DeepSeek 本地试用（进行中）

按用户选择，在现有模型端口后加入显式 DeepSeek 提供方，用于 CLI/工作台规划及只读探索的链接建议。隔离密钥和固定端点，不改变锚定、验证、审批或媒体事实。真实提供方试用需要用户在本机配置轮换后的密钥和获授权的测试 URL；未取得前只声明假客户端/离线验证通过。
