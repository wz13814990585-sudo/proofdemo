...
Stage 13 在执行和基础渲染后还会启动无网络的 Chromium 覆盖层绘制与 FFmpeg 重新编码。应预留额外 CPU、内存和磁盘；目前最多润色 12 个场景、24 个真实焦点提示。润色失败使工作台任务 `BLOCKED`，但不删除已验证的基础视频和执行证据；不要手工修改任务状态为成功。排查时检查 `job.json`、事件、基础视频、时间线与清单，确认 Chromium/FFmpeg 可用。`polish_plan.json` 的提示坐标是画面内相对位置，可能间接暴露应用界面布局；覆盖层、原视频和润色视频都按敏感产物保留。
